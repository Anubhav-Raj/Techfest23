  ...
  verifyRecaptcha(
    ...
    async () => {
      ...
      if (await User.findOne({ email: req.body.email })) {
        return res.status(208).send({
          title: "Error",
          message:
            "The email is already registered. Redirecting to signin page!",
          isError: true,
        });
      }
    ...
    (err) => {
      ...
      return res.status(500).json({
        isError: true,
        title: "Error, Failure in recaptcha",
  ...
  try {
    User.findOne({ email: email }, async (err, user) => {
      if (err || !user) {
        ...
        const uri = `https://www.techfestsliet.org/api/auth/reset-password/${
          user._id
        }/${token.replace(/\./g, "-")}`;
        transporter.sendMail(
          {
